...
class PaymentShell extends Shell {
    ...
    public function sendBillNotifications() {
        ...
        $conditions = [];
        $twoDayBefore = ['UploadedPaymentFiles.payment_cycle_date' => date('Y-m-d', strtotime(' +2 day'))];
        $oneDayBefore = ['UploadedPaymentFiles.payment_cycle_date' => date('Y-m-d', strtotime(' +1 day'))];
        $conditions = ['upload_count !=' => 0, 'OR' => [$twoDayBefore, $oneDayBefore]];
        $uploadedPaymentFiles = $this->UploadedPaymentFiles->find('all')->where($conditions)->contain([]);

        if ($uploadedPaymentFiles->count() > 0) {

            $mailTemplate = $this->MailTemplates->find()->where(['name' => 'PAYMENT_NOTIFICATION', 'is_active' => 1])->first();
            $adminSetting = $this->AdminSettings->find()->first();

            foreach ($uploadedPaymentFiles as $uploadedPaymentFile) {
                $payments = $this->Payments->find('all')->where(['Payments.uploaded_payment_file_id' => $uploadedPaymentFile->id, 'Payments.status' => 0])->contain(['Webfronts', 'Webfronts.Users']);
                if ($payments->count() > 0) {
                    foreach ($payments as $payment) {                      
                        ...
                        $this->Custom->sendEmail($payment->email, $adminSetting->from_email, $mailTemplate->subject, $message);
                        $this->Payments->query()->update()->set(['followup_counter' => 'followup_counter' + 1])->where(['id' => $payment->id])->execute();
