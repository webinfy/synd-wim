...
//$this->Auth->allow(['']);
    ...
    public function webfrontReport($id = NULL, $option = 0) {
        ...
        $objPHPExcel->getActiveSheet()->getStyle("F1:M1")->applyFromArray($style);
...
//Set Content
        ...
        foreach ($payments as $payment) {
            ...
            $objPHPExcel->getActiveSheet()->getStyle("F$rowCount:M$rowCount")->applyFromArray($style);
...
//Set Content
    ...
    public function ajaxViewPayments($uploadedPaymentFileId = NULL) {
        ...
        if (!empty($data['searchby'])) {
            ...
            } else if ($searchby == 'reference_number') {
                $conditions[] = ['Payments.reference_number' => $keyword];
            }
        }
        if ($data['status'] == 'paid') {
            $conditions[] = ['Payments.status' => 1];
    ...
    public function _sendReminderEmail($paymentId) {
        ...
        if ($query->count() > 0) {
            ...
            if (!empty($payment->email)) {
                ...
                $link = HTTP_ROOT . "customer/login/" . $payment->webfront->user->uniq_id;
