...
////////////////////////Prakash Code Starts Here\\\\\\\\\\\\\\\\\\\\\\\\
    ...
    public function ajaxEditMerchant($id = NULL) {
        ...
        if ($this->Users->save($this->MerchantProfiles->save($merchantProfile))) {
            ...
            echo json_encode(['status' => 'error', 'msg' => 'Some error occured. Please try later.']);
        }
        exit;
    }
...
// ***********************   Users Add Section by Merchant *********************
    ...
    public function ajaxAddUser() {
        ...
        if ($emailCount) {
            echo json_encode(['status' => 'error', 'msg' => 'Email not available. Please use another email.']);
            exit;
        }
        $phoneCount = $this->Users->find()->where(['Users.phone' => $phone, 'Users.type !=' => 4])->count();
        if ($phoneCount) {
            echo json_encode(['status' => 'error', 'msg' => 'Phone no not available. Please use another phone no.']);
            exit;
        }
        $user = $this->Users->newEntity();
        ...
        $user->type = 3;
        $user->is_active = 1;
        $user->access = !empty($data['user']['access']) ? $data['user']['access'] : 3;
