...
//                echo json_encode(['status' => 'error', 'msg' => "Excel file contain error. Please <a target='_blank' href='merchant/payments/download-error/" . $webfront->id . "'>Click here</a> to view it."]);
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                $query = $this->Users->find()->where(['Users.reference_number' => $reference_number, 'type' => 4]);
                if ($query->count() > 0) {
                    $user = $query->first();  //User Alreday Exist//
                } else {
//Create a new User//
                    $referenceNumberExist = $this->Users->find()->where(['Users.reference_number' => $reference_number])->count();
                ...
                if (!empty($user->id)) {
                    ...
                    $getPayment = $this->Payments->find()->where(['uploaded_payment_file_id' => $uploadedPaymentFile->id, 'status' => 0, 'reference_number' => $reference_number]);
                    if ($getPayment->count()) {
                        $payment->id = $getPayment->select(['id'])->first()->id;
                    } else {
                        $count = $this->Payments->find()->where(['uploaded_payment_file_id' => $uploadedPaymentFile->id, 'status' => 1, 'reference_number' => $reference_number])->count();
                        if ($count) {
                            continue;
                        }
                    }

//                    $count = $this->Payments->find()->where(['uploaded_payment_file_id' => $uploadedPaymentFile->id, 'reference_number' => $reference_number])->count();
//                    if (!$count) {
...
//                    }
    ...
    public function ajaxImportPayments() {
        ...
        if (date('Y-m-d') > date('Y-m-d', strtotime($data['payment_cycle_date']))) {
            echo json_encode(['status' => 'error', 'msg' => 'Payment Cycle Date should not less that current date.']);
            exit;
        }
...
//Header
        ...
        if ($this->_hasDuplicatedValues($objPHPExcel, 'B')) {
            $txt = "File contains duplicate entry for Reference No \n";
            fwrite($logFile, $txt);
            fclose($logFile); //Close Log file
            return ['status' => 'error', 'msg' => $txt];
            exit;
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if ($reference_number == "") {
                $txt = "Reference number is empty at row {$row}  \n";
                fwrite($logFile, $txt);
                $errorFound = TRUE;
            }
    ...
    public function _sendReminderEmail($paymentId) {
        $query = $this->Payments->find('all')->where(['Payments.id' => $paymentId])->contain(['Users', 'Webfronts', 'Webfronts.MerchantProfiles']);
        if ($query->count() > 0) {
            $payment = $query->first();
            ...
            $webfrontUrl = $payment->webfront->url;
            $link = HTTP_ROOT . "customer/payments/make-payment/" . $uniqId;
            $directLink = HTTP_ROOT . "customer/webfronts/payment/$webfrontUrl/$uniqId";
            $viewTransLink = HTTP_ROOT . "customer/view-transactions/$uniqId";
            $message = $this->Custom->formatEmail($mailTemplate['content'], [
                'NAME' => $payment->name,
                'EMAIL' => $payment->email,
                'PHONE' => $payment->phone,
                'MERCHANT' => $payment->webfront->merchant_profile->name,
                'FEE' => " Rs." . $payment->fee,
                'CONVENIENCE_FEE' => " Rs." . $payment->convenience_fee_amount,
                'BILL_AMOUNT' => " Rs." . ($payment->fee + $payment->convenience_fee_amount),
                'LATE_FEE_AMOUNT' => " Rs." . $payment->webfront->late_fee_amount,
                'PAYMENT_CYCLE_DATE' => date("d M, Y", strtotime($payment->webfront->payment_cycle_date)),
                'MONTH' => date("F", strtotime($payment->webfront->payment_cycle_date)),
...
//SetHeading//   
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue($head[$count++] . '1', $webfront->customer_reference_number_alias);
    ...
    public function create() {
        ...
        $webfront->fixed_late_fee = !empty($data['webfront']['fixed_late_fee']) ? $data['webfront']['fixed_late_fee'] : 1;
        ...
        if ($this->Webfronts->save($webfront)) {
            ...
            if (!empty($data['croppedImage'])) {
                ...
                if ($data) {
                    ...
                    file_put_contents(WEBFRONT_LOGO . $logo, $data);
    ...
    public function edit() {
        ...
        if ($this->Webfronts->save($webfront)) {
            if (!empty($data['croppedImage'])) {
                ...
                if ($data) {
                    /* Delete Old Logo */
                    if ($webfront->logo != '' && file_exists(WEBFRONT_LOGO . $webfront->logo)) {
    ...
    public function ajaxUpdatePayeeFields() {
        ...
        $fields['customer_reference_number_alias'] = !empty($data['webfront']['customer_reference_number_alias']) ? $data['webfront']['customer_reference_number_alias'] : 'Reference Number';
    ...
    public function ajaxAdvanceGetDetails($id) {
        ...
        if ($query->count() > 0) {
            ...
            $total_amount = $query->select([
                    ...
                    ])->where(['webfront_id' => $webfront->id])->toArray();
...
//Payment attribute section starts here
    public function ajaxAddPaymentAttribute() {
        ...
        if ($this->WebfrontPaymentAttributes->save($webfrontPaymentAttribute)) {
            ...
            $total_amount = $query->select([
                        'total_price' => $query->func()->sum('value')
                    ])->where(['webfront_id' => $data['webfront_id']])->toArray();
    ...
    public function ajaxUpdatePaymentAttribute() {
        ...
        if ($this->WebfrontPaymentAttributes->save($webfrontPaymentAttribute)) {
            ...
            $total_amount = $query->select([
                        'total_price' => $query->func()->sum('value')
                    ])->where(['webfront_id' => $data['payment_attribute']['webfront_id']])->toArray();
            echo json_encode(['status' => 'success', 'webfront_payment_attributes' => $webfront_payment_attributes, 'total_amount' => $total_amount[0]['total_price']]);
            exit;
        }
        echo json_encode(['status' => 'error', 'msg' => 'Some error occured. Please try again!!']);
