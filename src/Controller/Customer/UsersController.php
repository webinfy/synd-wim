...
//        pr($this->request->session()->read());
    ...
    public function login($uniqId = NULL, $title = NULL) {
        ...
        if ($this->request->is('post')) {
            ...
            $this->Auth->config('authenticate', [
                'Form' => [
                    'fields' => ['username' => 'reference_number']
                ]
            ]);
            ...
            if (!$user) {
                $this->Flash->error(__("Invalid Reference Number or Password, try again"));
                return $this->redirect($this->referer());
                exit;
            }
            ...
            $this->loadModel('Payments');
            $webfrontList = $this->Webfronts->find('list', ['keyField' => 'id', 'valueField' => 'id'])->where(['Webfronts.merchant_id' => $merchant->id])->toArray();
            $conditions = ['Payments.customer_id' => $user['id'], 'Payments.webfront_id IN' => $webfrontList];
            $customerExist = $this->Payments->find('all')->where($conditions);
            if ($customerExist->count() <= 0) {
                $this->Flash->error(__("Un-athorized Access."));
                return $this->redirect($this->referer());
                exit;
            }
    ...
    public function logout($merchantUniqId = NULL) {
        if ($this->request->session()->read('merchant.uniq_id')) {
            $redirectUrl = HTTP_ROOT . "customer/login/" . $this->request->session()->read('merchant.uniq_id');
        } else {
            $redirectUrl = HTTP_ROOT;
        }
        $this->Auth->logout();
