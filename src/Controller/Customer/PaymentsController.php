...
class PaymentsController extends AppController {
    ...
    public function confirmLogin($uniqId = NULL) {
        ...
        $merchant = $this->Users->find()->where(['Users.id' => $payment->webfront->merchant_id])->contain(['MerchantProfiles'])->first();
    ...
    public function ajaxViewPaymentDetails($uniqId) {
        $this->viewBuilder()->layout('ajax');
        $conditions = ['Payments.customer_id' => $this->Auth->user('id'), 'Payments.status' => 0, 'Payments.uniq_id' => $uniqId];
    ...
    ';
        ...
        $netBill = $payment->fee + $payment->late_fee_amount;

        $email = !empty($payment->email) ? $payment->email : 'NA';
        $phone = !empty($payment->phone) ? $payment->phone : 'NA';
        ...
        $message .= "<p>Reference No. : {$payment->reference_number}</p><br/>";
        $message .= "<p>Customer Email :{$email}</p><br/>";
        $message .= "<p>Customer Phone : {$phone}</p><br/>";
        ...
        $message .= "<p>&nbsp;</p>";
        $payment_custom_fields = json_decode($payment->payment_custom_fields, true);
        if (count($payment_custom_fields)) {
            foreach ($payment_custom_fields as $payment_custom_field) {
                $message .= "<p>{$payment_custom_field['field']} : Rs. " . $payment_custom_field['value'] . "</p><br/>";
            }
        }
        if (!empty($payment->late_fee_amount)) {
            $message .= "<p>Late Fee : Rs. " . $payment->late_fee_amount . "</p><br/>";
        }
        $message .= "<p>Net Bill Amount : Rs. " . $netBill . "</p><br/>";
        $message .= "<p>Convenience Fee : Rs. " . $payment->convenience_fee_amount . "</p>";
        $message .= "<p>&nbsp;</p>";
    ...
    public function viewDetails($uniq = null) {
        ...
        if ($query->count() > 0) {
            ...
            //$merchant = $this->Users->find()->where(['Users.id' => $payment->uploaded_payment_file->webfront->merchant_id])->contain(['MerchantProfiles'])->first();
            $this->set(compact('payment'));
        } else {
            return $this->redirect(HTTP_ROOT);
        }
    ...
    public function ajaxViewTransactions($uniq = NULL) {
        ...
        $reference_number = $data['phone'];
        $payments = $this->Payments->find('all')->where(['Payments.reference_number' => $reference_number, 'Webfronts.merchant_id' => $merchantId])->contain(['Users', 'Webfronts', 'UploadedPaymentFiles'])->order(['Payments.id' => 'DESC']);
        if ($payments->count() <= 0) {
            echo json_encode(['status' => 'error', 'msg' => 'Invalid Reference No.']);
            exit;
        }
    ...
    public function success2($uniq = null) {
        ...
        $query = $this->Payments->find()->where(['Payments.uniq_id' => $uniq]);
        if ($query->count() > 0) {
            $payment = $query->contain(['Webfronts', 'Webfronts.Users'])->first();
            $this->set(compact('payment'));
        } else {
            return $this->redirect(HTTP_ROOT);
        }
